...
public sealed class ImportBatch : AuditableEntity
{
    ...
    {
        if (Status != ImportBatchStatus.Uploaded) throw new InvalidOperationException($"Import batch cannot begin validation from status {Status}.");
    ...
    {
        if (Status is not (ImportBatchStatus.Uploaded or ImportBatchStatus.Validating)) throw new InvalidOperationException($"Import batch cannot fail validation from status {Status}.");
        Status = ImportBatchStatus.ValidationFailed;
    }

    public void AcceptForCommit(string actor, string? acceptanceReason = null)
    {
        if (Status != ImportBatchStatus.PreviewReady) throw new InvalidOperationException($"Import batch cannot be accepted from status {Status}.");
        if (string.IsNullOrWhiteSpace(actor)) throw new ArgumentException("Actor is required.", nameof(actor));
        if (ReconciledToExpectedTargets != true && string.IsNullOrWhiteSpace(acceptanceReason)) throw new InvalidOperationException("A reason is required when reconciliation targets are not met.");
    ...
    {
        if (Status != ImportBatchStatus.PreviewReady) throw new InvalidOperationException($"Import batch cannot be committed from status {Status}.");
        if (string.IsNullOrWhiteSpace(AcceptedBy)) throw new InvalidOperationException("Import batch must be explicitly accepted before commit.");
