...
namespace Crch.ItBudget.Infrastructure.Importing;

public sealed record Fy2027ImportPreviewResult(
    Guid ImportBatchId,
...
public sealed class Fy2027ImportPreviewService(
    ItBudgetDbContext dbContext,
    Fy2027WorkbookReader workbookReader)
{
    private const string ImportType = "FY2027_WORKBOOK";
    private const decimal PlannedTotalTolerance = 0.005m;

    public async Task<Fy2027ImportPreviewResult> CreatePreviewAsync(
        Stream source,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            buffer.Position = 0;
            var workbook = workbookReader.Read(buffer, sourceFileName);
            ...
            foreach (var sourceRow in workbook.BudgetRows)
            {
                var importRow = new ImportRow(
                    batch.Id,
                    Fy2027WorkbookSchema.RawBudgetInfoSheet,
                    sourceRow.SourceRowNumber,
                    $"FY2027:{sourceRow.ItemNumber}",
            ...
            if (!workbook.Reconciliation.MeetsTargets)
            {
                dbContext.ImportExceptions.Add(new ImportException(
                    batch.Id,
                    null,
                    "RECONCILIATION_TARGET_MISMATCH",
                    "The workbook does not reconcile to the expected FY2027 item count, planned total, Must Have count, and row-level total checks. Explicit administrator acceptance with a reason is required before commit.",
                    ImportExceptionSeverity.Warning));
                warningCount++;
                exceptionCount++;
            }
            ...
            batch.CompletePreview(
                ...
                workbook.Reconciliation.MustHaveCount,
                workbook.Reconciliation.MeetsTargets,
        ...
        catch (Fy2027WorkbookValidationException exception)
        {
        ...
        IReadOnlyCollection<LookupValueAlias> aliases)
    {
        ...
        if (workflowFlags > 1)
        {
            issues.Add(new ImportException(
                importBatchId,
                importRowId,
                "CONTRADICTORY_WORKFLOW_FLAGS",
                $"Item {row.ItemNumber} has more than one of Approval, Denial, and Proposed set to TRUE.",
                ImportExceptionSeverity.Error));
        }
        ...
        if (Math.Abs(row.RecalculatedPlannedTotal - row.SourcePlannedTotal) > PlannedTotalTolerance)
        {
            issues.Add(new ImportException(
                ...
                $"Item {row.ItemNumber} source total {row.SourcePlannedTotal:0.####} recalculates to {row.RecalculatedPlannedTotal:0.####}; the recalculated value will be authoritative.",
                ImportExceptionSeverity.Warning));
        }
        ...
        if (string.IsNullOrWhiteSpace(row.Vendor))
        {
            issues.Add(new ImportException(
                ...
                importRowId,
                "MISSING_VENDOR",
                $"Item {row.ItemNumber} does not identify a vendor. The item may be imported, but vendor assignment should be reviewed.",
                ImportExceptionSeverity.Warning));
        }
    ...
    {
        if (TryResolveLookup(lookupType, sourceValue, index, parseLeadingCode, aliases, out _))
        {
            return;
        }
        ...
        issues.Add(new ImportException(
            importBatchId,
            importRowId,
            $"UNKNOWN_{lookupType.ToUpperInvariant()}",
            $"Source value '{sourceValue}' does not map to an active {lookupType} lookup value.",
            ImportExceptionSeverity.Error));
    }
        ...
        out string? canonicalCode)
    {
        ...
        if (index.Names.TryGetValue(sourceValue.Trim(), out canonicalCode))
        {
            return true;
        }
    ...
    {
        public static async Task<LookupSnapshot> LoadAsync(
            ItBudgetDbContext dbContext,
