...
namespace Crch.ItBudget.Application.Budgeting;
...
public static class BudgetAllocationReconciler
{
    ...
    public static BudgetAllocationReconciliation Reconcile(
        decimal expectedAmount,
        IReadOnlyCollection<BudgetItemAllocation> allocations)
    {
        ...
        if (method == AllocationMethod.Percentage)
        {
            ...
            var calculatedAmount = allocations.Sum(x =>
                Math.Round(expectedAmount * (x.Percentage ?? 0m) / 100m, 4, MidpointRounding.AwayFromZero));
            var difference = expectedAmount - calculatedAmount;

            if (Math.Abs(100m - percentageTotal) > PercentageTolerance)
            {
                errors.Add($"Allocation percentages total {percentageTotal:0.####}% instead of 100%.");
            }

            if (Math.Abs(difference) > CurrencyTolerance)
            {
                errors.Add($"Percentage allocation rounding leaves an unreconciled difference of {difference:0.0000}.");
            }
        ...
        if (Math.Abs(amountDifference) > CurrencyTolerance)
        {
            errors.Add($"Allocation amounts total {amountTotal:0.0000} instead of {expectedAmount:0.0000}.");
        }
